function [edge_final] = canny_edge(pic)
img = pic;  %Input image
figure, imshow(img);
img = double (img);
T_Low = 0.075;   %Value for Thresholding
T_High = 0.175;
B = [2, 4, 5, 4, 2; 4, 9, 12, 9, 4;5, 12, 15, 12, 5;4, 9, 12, 9, 4;2, 4, 5, 4, 2 ]; %Gaussian Filter Coefficient
B = 1/159.* B;
A=conv2(img, B, 'same');   %Convolution of image with Gaussian Coefficient

KGx = [-1, 0, 1; -2, 0, 2; -1, 0, 1];%Filter for horizontal direction
KGy = [1, 2, 1; 0, 0, 0; -1, -2, -1];%Filter for vertical direction
Filtered_X = conv2(A, KGx, 'same');%Convolution by image by horizontal filter
Filtered_Y = conv2(A, KGy, 'same');%Convolution by image by vertical filter
